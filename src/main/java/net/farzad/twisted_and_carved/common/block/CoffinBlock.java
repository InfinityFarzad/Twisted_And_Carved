...
public class CoffinBlock extends BlockWithEntity {
        ...
        if (!world.getBlockState(pos).get(OPEN) && player.getMainHandStack().isEmpty()) {
            if (world instanceof ServerWorld server && server.getBlockState(pos).get(FIRST_OPENING)) {
                if (world instanceof ServerWorld serverWorld) {
                    serverWorld.spawnParticles(TCParticles.COFFIN_SMOKE,pos.getX() + 0.5,pos.getY() + 1.1,pos.getZ() + 0.5,14, 0,0,0,0.08);
                }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (state.get(OPEN)) {
            world.addParticleClient(TCParticles.COFFIN_ASH, x + Math.max(random.nextDouble() / 2,0.28) * (random.nextBoolean() ? 1 : -1), y + Math.max(random.nextDouble() / 20,0.15) * (random.nextBoolean() ? 1 : -1), z + Math.max(random.nextDouble() / 2,0.28) * (random.nextBoolean() ? 1 : -1), 0, 0, 0);
            world.addParticleClient(TCParticles.COFFIN_ASH, x + Math.max(random.nextDouble() / 2,0.28) * (random.nextBoolean() ? 1 : -1), y + Math.max(random.nextDouble() / 20,0.15) * (random.nextBoolean() ? 1 : -1), z + Math.max(random.nextDouble() / 2,0.28) * (random.nextBoolean() ? 1 : -1), 0, 0, 0);
            world.addParticleClient(TCParticles.COFFIN_ASH, x + Math.max(random.nextDouble() / 2,0.28) * (random.nextBoolean() ? 1 : -1), y + Math.max(random.nextDouble() / 20,0.15) * (random.nextBoolean() ? 1 : -1), z + Math.max(random.nextDouble() / 2,0.28) * (random.nextBoolean() ? 1 : -1), 0, 0, 0);
        }
