            ...
            if (user.getEntityWorld() instanceof ServerWorld serverWorld) {
                ...
                for (Entity entity : entities) {
                    if (entity instanceof LivingEntity livingEntity) {
                        double distance = entity.getEntityPos().distanceTo(user.getEntityPos());
                        livingEntity.damage(serverWorld, user.getDamageSources().create(TCDamageTypes.SWEEPING_SLASH,user), (float) (4 / distance * 2.5f));
                        livingEntity.takeKnockback(0.05 * distance,user.getX(), user.getY());
                    }
                }
                serverWorld.spawnParticles(TCParticles.TWISTED_GLAIVE_SWEEP, user.getX(), user.getY() + 1.0, user.getZ(), 1, 0, 0, 0, 1);
                serverWorld.playSound(null, user.getX(), user.getY(), user.getZ(), TDSounds.TWISTED_GLAIVE_SWEEP, user.getSoundCategory(), 8.0F, 1.0F);
